...
function cleanAmount(amount: string | number): number {
    ...
    if (typeof amount === 'string') {
        const cleaned = amount.replace(/\$/, '').replace(/\(/, '-').replace(/\)/, '').replace(/,/, '');
        return parseFloat(cleaned) || 0.0;
    }
    return 0.0;
}

function parseOptionDescription(description: string): [string | null, string | null, string | null, number | null] {
    if (typeof description !== 'string') {
        return [null, null, null, null];
    }
    const match = description.match(/([\w\.]+)\s+(\d{1,2}\/\d{1,2}\/\d{4})\s+(Call|Put)\s+\$([\d\.]+)/);
    if (match) {
...
function processTrades(df: Trade[]): CompletedTrade[] {
    ...
    // --- Options Processing ---
    const optionsDf = df.filter(row => ['BTO', 'STC', 'OEXP'].includes(row['Trans Code']));
    ...
    optionsDf.sort((a, b) => {
        ...
        return (transCodeOrder[a['Trans Code'] as keyof typeof transCodeOrder] || 99) - (transCodeOrder[b['Trans Code'] as keyof typeof transCodeOrder] || 99);
    });

    optionsDf.forEach(row => {
        const [optionTicker, , optionType] = parseOptionDescription(row.Description);
        ...
        if (row['Trans Code'] === 'BTO') {
            ...
            while (sellQtyRemaining > 0 && openOptions[key] && openOptions[key].length > 0) {
                const openPos = openOptions[key][0];
                const qtyToSell = Math.min(sellQtyRemaining, openPos.quantity);

                const costBasisPerContract = openPos.quantity !== 0 ? openPos.amount / openPos.quantity : 0;
                const costBasisForLot = costBasisPerContract * qtyToSell;

                const proceedsPerContract = row.Quantity !== 0 ? sellAmount / row.Quantity : 0;
                ...
                const [, , , strikePrice] = parseOptionDescription(row.Description);
                ...
                completedOptions.push({
                    ...
                    holding_period: Math.round((sellDate.getTime() - openPos.date.getTime()) / (1000 * 60 * 60 * 24)),
    ...
    const stocksDf = df.filter(row => ['Buy', 'Sell'].includes(row['Trans Code']));
    ...
    const stockTransCodeOrder = { 'Buy': 1, 'Sell': 2 };
     stocksDf.sort((a, b) => {
        ...
        const dateB = new Date(b['Activity Date']).getTime();
        if (dateA !== dateB) return dateA - dateB;
        return (stockTransCodeOrder[a['Trans Code'] as keyof typeof stockTransCodeOrder] || 99) - (stockTransCodeOrder[b['Trans Code'] as keyof typeof stockTransCodeOrder] || 99);
    });
    ...
    stocksDf.forEach(row => {
        ...
        if (row['Trans Code'] === 'Buy') {
            ...
            while (sellQtyRemaining > 0 && openStocks[symbol] && openStocks[symbol].length > 0) {
                ...
                completedStocks.push({
                    ...
                    holding_period: Math.round((sellDate.getTime() - buyPos.date.getTime()) / (1000 * 60 * 60 * 24)),
                    type: 'Stock',
                    pnl: pnlForThisLot,
                    status: status,
                });
...
async function loadAndProcessData(): Promise<CompletedTrade[]> {
    ...
    const filePath = path.join(process.cwd(), '..', 'Feb 22, 2017 – Mar 25, 2025.csv');
